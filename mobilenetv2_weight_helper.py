...
def load_pretrained_imagenet_resnext_weights(model):
    ...
    weights_file = os.path.join(cfg['ROOT_DIR'], cfg['MODEL_REPOSITORY'], 'MobileNetV2-ImageNet', cfg['PRETRAINED_WEIGHTS'])
    ...
    for k, v in pretrianed_state_dict.items():
        if k in model_state_dict.keys():
            ...
            print('Weight %s is not in MobileNetV2 model.' % k)
            #logger.info('Weight %s is not in MobileNetV2 model.' % k)
    print('Pretrained MobileNetV2 weight has been loaded')
